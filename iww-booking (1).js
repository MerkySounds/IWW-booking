...
#iww-root{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;max-width:680px;padding:1rem 0;}
...
#iww-open-btn:hover{opacity:.88;transform:translateY(-2px);}
...
#iww-root .loading-bar{text-align:center;padding:1rem;font-size:13px;color:#888;grid-column:1/-1;}
`;

const HTML = `
<div id="iww-root">
  <button id="iww-open-btn">🎵 Book a slot — I Wub Wednesdays</button>

  <div id="iww-form-wrapper">
...
// Toggle button
const openBtn = document.getElementById('iww-open-btn');
const wrapper = document.getElementById('iww-form-wrapper');
let formOpen = false;
openBtn.onclick = () => {
  formOpen = !formOpen;
  wrapper.classList.toggle('open', formOpen);
  openBtn.textContent = formOpen ? '✕ Close booking form' : '🎵 Book a slot — I Wub Wednesdays';
  if (formOpen) {
...
async function fetchTakenSlots(dateFull) {
  ...
  results.forEach(({ slot, taken }) => { takenSlots[dateFull][slot] = taken; });
}
...
async function selectDate(w) {
  ...
  document.getElementById('iww-slots-grid').innerHTML = '<div class="loading-bar">Checking availability...</div>';
